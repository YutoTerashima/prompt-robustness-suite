...
"""Project-specific research helpers for Prompt Robustness Suite.

Purpose: Perturbation families for casing, typoglycemia, wrapping, and instruction sandwich tests.
...
MODULE_PURPOSE = 'Perturbation families for casing, typoglycemia, wrapping, and instruction sandwich tests.'
...
def perturb(rows: Iterable[Mapping[str, Any]]) -> list[dict[str, Any]]:
...
def perturbation_matrix(rows: Iterable[Mapping[str, Any]], label: str = "experiment_id") -> dict[str, float]:
    """Summarize the project-specific tradeoff metrics used in the report."""
    materialized = [dict(row) for row in rows]
    if not materialized:
        return {"rows": 0.0, "best_quality": 0.0, "mean_quality": 0.0}
    ranked = perturb(materialized)
...
def family_labels(records: Iterable[Mapping[str, Any]], field: str = "failure_type") -> list[dict[str, Any]]:
