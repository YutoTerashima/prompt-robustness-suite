...
Purpose: Prompt policy A/B/C definitions and detector-routing helpers.
...
MODULE_PURPOSE = 'Prompt policy A/B/C definitions and detector-routing helpers.'
...
def policy_keywords(rows: Iterable[Mapping[str, Any]]) -> list[dict[str, Any]]:
...
def evaluate_policy(rows: Iterable[Mapping[str, Any]], label: str = "experiment_id") -> dict[str, float]:
    """Summarize the project-specific tradeoff metrics used in the report."""
    materialized = [dict(row) for row in rows]
    if not materialized:
        return {"rows": 0.0, "best_quality": 0.0, "mean_quality": 0.0}
    ranked = policy_keywords(materialized)
...
def policy_tradeoff(records: Iterable[Mapping[str, Any]], field: str = "failure_type") -> list[dict[str, Any]]:
