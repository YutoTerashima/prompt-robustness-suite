...
Purpose: Attack-recall and benign-pass-rate gates for prompt regression testing.
...
MODULE_PURPOSE = 'Attack-recall and benign-pass-rate gates for prompt regression testing.'
...
def gate_result(rows: Iterable[Mapping[str, Any]]) -> list[dict[str, Any]]:
...
def recommended_policy(rows: Iterable[Mapping[str, Any]], label: str = "experiment_id") -> dict[str, float]:
    """Summarize the project-specific tradeoff metrics used in the report."""
    materialized = [dict(row) for row in rows]
    if not materialized:
        return {"rows": 0.0, "best_quality": 0.0, "mean_quality": 0.0}
    ranked = gate_result(materialized)
...
def leaderboard_summary(records: Iterable[Mapping[str, Any]], field: str = "failure_type") -> list[dict[str, Any]]:
